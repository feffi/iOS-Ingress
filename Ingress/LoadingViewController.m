		...
		if ([cookie.name isEqualToString:@"SACSID"]) {
			...
			[[API sharedInstance] processHandshakeData:jsonString completionHandler:^(NSString *errorStr) {
				if (!errorStr) {
					...
					if ([errorStr isEqualToString:@"Activation needed for playing"]) {
					
						UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Activation code" message:nil delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Activate", nil];
						[alertView setAlertViewStyle:UIAlertViewStylePlainTextInput];
						[alertView setTag:1];
						[alertView show];
						
					} else if ([errorStr isEqualToString:@"You must accept TOS"]) {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	switch (alertView.tag) {
		case 1:
			...
			if (buttonIndex == 1) {
				UITextField *textField = [alertView textFieldAtIndex:0];
				[jsonDict setValue:textField.text forKey:@"activationCode"];
				[self performHandshake];
