...
- (void)updateIndicators {
	...
	BOOL isActivityIndicator = [indicator isKindOfClass:[WheelActivityIndicatorView class]]; //UIActivityIndicatorView
...
//		[(UIActivityIndicatorView *)indicator startAnimating];
		self.indicator = MB_AUTORELEASE([[WheelActivityIndicatorView alloc]
										 initWithFrame:CGRectZero]);
		[(WheelActivityIndicatorView *)indicator startAnimating];
...
- (void)layoutSubviews {
	...
	if (!self.showCloseButton && closeBtn) {
		...
		CGRect boxRect = CGRectMake(roundf((allRect.size.width - self.size.width) / 2) + self.xOffset, roundf((allRect.size.height - self.size.height) / 2) + self.yOffset, self.size.width, self.size.height);
		[closeBtn setFrame:CGRectMake(boxRect.origin.x-10, boxRect.origin.y-10, 29, 29)];
