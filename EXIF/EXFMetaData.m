...
-(int) processExifDir:(NSMutableDictionary*) keyedValues: (int) dirStart: (int) offsetBase: (BOOL) thumbnail{
    ...
    for (int de =0;de <numEntries;de++){
        ...
        if (byteCount > 4) {
            ...
            Debug(@"Offset %li found for tag %i with bytecount %li",valueOffset, tag,byteCount);
       ...
       // sometimes thumbnail has no tag just runs on end of data - so keep track of where we are in block
        ...
        if (tag == TAG_EXIF_OFFSET || tag == TAG_INTEROP_OFFSET || tag == TAG_EXIF_GPS) {
            ...
            Debug(@"Nested pointer to %li found for tag %i with bytecount %li",subdirOffset +offsetBase, tag,byteCount);
            ...
            if (handler != nil){
                Debug(@"Handler %@ invoked for tag %@ invoked with offset %li and byte count %li",handler, tagNumber, valueOffset, byteCount);
                ...
                @try {
                    ...
                    [handler decodeTag: keyedValues: tagNumber:(CFDataRef*) &tagData: self.bigEndianOrder];
                                   ...
                                   NSLog(@"Unable to process Tag %@ due to error %@", tagNumber,  theError);
    ...
    Debug(@"Next Offset %li at %i",nextOffset, nextIFD);
    ...
    if(thumbnail){
      ...
      Debug(@"********** Leaving exif processing at %i with nextOffset %li***********",dirStart  , nextOffset);
    return nextOffset;
}
